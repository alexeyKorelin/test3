...
class ScrollBar extends React.Component {
    ...
    calculateState(props){
        let scrollBarRatio = (props.scrollBarSize && props.scrollBarSize < props.containerSize) ? props.scrollBarSize / props.containerSize : 1; 
        let relativeRealSize = props.realSize * scrollBarRatio;
        let relativeContainerSize = props.containerSize * scrollBarRatio;
        let relativePosition = props.position * scrollBarRatio;
        let fractionalPosition = this.calculateFractionalPosition(relativeRealSize, relativeContainerSize, relativePosition);
        let proportionalToContainerScrollSize = relativeContainerSize * relativeContainerSize / relativeRealSize;
        let scrollSize = proportionalToContainerScrollSize < props.minScrollSize ? props.minScrollSize : proportionalToContainerScrollSize;
        let scrollPosition = (relativeContainerSize - scrollSize) * fractionalPosition;
        return {
    ...
    render(){
        let {smoothScrolling, isDragging, type, scrollbarStyle, containerStyle, containerClassName, scrollBarClassName, scrollBarSize} = this.props;
        ...
        let scrollbarClasses = `scrollbar-container ${isDragging ? 'active' : ''} ${isVoriziontal ? 'horizontal' : ''} ${isVertical ? 'vertical' : ''} ${containerClassName ? containerClassName : ''}`;
            ...
            <Motion style={springifiedScrollStyles}>
                { style =>
                    <div
                        ...
                        style={{ maxWidth: scrollBarSize, ...containerStyle}}
                        onMouseDown={this.handleScrollBarContainerClick.bind(this)}
                        ref={ x => this.scrollbarContainer = x }
                    >
                        <div
                            className={`scrollbar ${scrollBarClassName ? scrollBarClassName : ''}`}
